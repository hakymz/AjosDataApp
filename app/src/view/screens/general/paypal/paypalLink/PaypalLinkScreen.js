...
const generateLink = async (data, navigation) => {
  try {
    const response = await fetchRequest({
    ...
    if (response?.status == 'success' && response?.data) {
      navigation.navigate('PaypalLinkNextScreen', {...response?.data});
    }
  } catch (error) {
...
export const PaypalLinkScreen = ({navigation}) => {
  ...
  const {
    ...
    onSubmit: values => {
      generateLink({...values}, navigation);
    },
  });
