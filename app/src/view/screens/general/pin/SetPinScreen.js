  ...
  const addOrDeletePin = (number, state, setState) => {
    ...
    if (number == 'Cancel') {
      ...
      if (number) {
        ...
        if (pin.length == 4) {
          if (currentPin?.length == 0 && pin.length == 4) {
            currentPin = pin?.join('');
  ...
  const setPin = async () => {
    try {
      const response = await fetchRequest({
        path: 'settings/set-transaction-pin',
        data: {
      ...
      navigation?.replace('PinSuccessScreen');
  ...
  return (
    <CustomSafeAreaView style={{flex: 1, backgroundColor: COLORS.white}}>
      ...
      <KeyboardAvoidingViewWrapper
        ...
        <View>
          <Text color={'#868D95'}>Hello, Bonjour, Hola, 你好 ...</Text>
        ...
        <View style={{flex: 1, justifyContent: 'flex-end'}}>
          <Text
            ...
            {state?.error
              ? state?.error
              : state?.currentPin?.length > 3
              ? 'Confirm your PIN'
              : 'Create your 4-digit unique PIN'}
          </Text>
          <FlatList
            data={numberList}
