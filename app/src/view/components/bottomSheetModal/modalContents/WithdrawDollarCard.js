...
export const WithdrawDollarCard = ({rate, card}) => {
  ...
  const withdrawFunds = async transactionPin => {
    try {
      ...
      Toast.show('success', 'Withdrawal was successful.');
    ...
    <View>
      ...
      <View
        ...
        <Text
          ...
          style={{opacity: 0.5}}
          color={COLORS.black}>
          Conversion rate: $1 = ₦{formatAmount(rate?.conversionRate)}{' '}
        </Text>
        <Text
          size={12}
          semiBold
        ...
        <Input
          ...
          value={values.amount * rate?.conversionRate}
          // error={touched?.amount && errors?.amount}
          placeholder="0.0"
          inputStyle={{
            textAlign: 'right',
